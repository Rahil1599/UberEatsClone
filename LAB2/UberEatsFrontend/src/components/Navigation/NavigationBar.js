...
import { updateCart, setMode, clearCart } from '../../state/action-creators/cartActionCreator';
import setAllDishes from '../../state/action-creators/restaurantActionCreator';
import clearRestaurant from '../../state/action-creators/restaurantActionCreator';
...
export default function NavigationBar(props) {
  ...
  const restaurantMode = useSelector((state) => state.restaurant.currentRestaurant.Mode);
  ...
  const onRemoveFromCart = (dish) => {
    const newCart = [...cart];
    const index = newCart.findIndex((item) => item._id === dish._id);
    if (index === -1) return;
          ...
          <AppBar style={style} position="static">
            <Toolbar style={{ display: 'flex', width: '100%' }}>
              ...
              <Box sx={{ display: { xs: 'none', md: 'flex' } }}>
                ...
                {(props.view === 'customerdashboard' || props.view === 'customerrestaurant') && (
