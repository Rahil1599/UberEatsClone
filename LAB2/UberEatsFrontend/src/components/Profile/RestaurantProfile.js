...
export default function RestaurantProfile() {
  ...
  const isValid = (payload) => {
    ...
    if (!nameRegex.test(payload.RestaurantName)) {
      setNameError(true);
    ...
    if (payload.WorkHrsFrom >= payload.WorkHrsTo) {
  ...
  const handleSubmit = async (event) => {
    ...
    const payload = {
      ...
      RestaurantDesc: data.get('desc'),
      Country: data.get('country'),
      State: data.get('state'),
      Pincode: data.get('pincode'),
      City: data.get('city'),
      WorkHrsFrom: data.get('fromHrs'),
      WorkHrsTo: data.get('toHrs'),
