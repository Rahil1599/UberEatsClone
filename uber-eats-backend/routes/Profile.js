...
router.post("/restaurant/:id",async(req,resp)=>{
    ...
    let query = "UPDATE Restaurant SET RestaurantName = ?, RestaurantDesc = ?, Country = ?, City = ?, State = ?, PhoneNumber = ?,Pincode = ?, WorkHrsFrom = ? , WorkHrsTo = ?, ImageUrl = ? where RestaurantId = ?";
    dbPool.query(query,[restaurantName,desc,country,city,state,phone,pincode,fromHrs,toHrs,imageUrl,restaurantId],function(err,results, fields){
        if(err){
          console.log(err);
